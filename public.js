async function buttonLogic() {
  ...
  mintButton.addEventListener("click", async (_) => {
    if (provider) {
      if (!account) {
        handleConnectWallet();
      } else {
       ...
       // const currentSupply = document.getElementById("currentSupply");
        ...
        const transData = {
          //set the gas limit to be a base cost of 200000 + 10000 for each token minted
          gasLimit: 200000 + ((150000) * (mintAmnt)),
          from: account,
          value: 250000000000000000 * mintAmnt,
        };
        contract.methods.Mint(mintAmnt).send(transData);
      }
    } else {
      window.location = "https://metamask.io/";
    }
